...
const QuestionEdit= ()=> {
   ...
   useEffect(() => {
    fetch("http://localhost:3005/questions/" + qid).then((res) => {
        return res.json();
    }).then((resp) => {
        idchange(resp.id);
        topicchange(resp.topic_id);
        questionchange(resp.question)
        answer1change(resp.answer1)
        answer2change(resp.answer2)
        answer3change(resp.answer3)

        correct_answerschange(resp.correct_answers)
        imagechange(resp.images)
        pointschange(resp.points)
        timechange(resp.timer_seconds)
        activechange(resp.is_active)
